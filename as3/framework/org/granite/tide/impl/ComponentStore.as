...
package org.granite.tide.impl { 
	...
	public class ComponentStore {
		...
		public function getComponentTypes(type:Type):Array {
			var types:Array = [type.name];
			for each (var ec:Type in type.superclasses)
				types.push(ec.name);
			for each (var ii:Type in type.interfaces)
				types.push(ii.name);
		...
		public function internalAddComponent(name:String, type:Type, properties:Object, inConversation:Boolean = false, autoCreate:Boolean = true, restrict:int = 0, 
			...
			var isRemoteProxy:Boolean = type.isSubclassOf(Tide.TYPE_ICOMPONENT);
            var isGlobal:Boolean = (
				type === Tide.TYPE_SUBCONTEXT || isRemoteProxy ||
				(type.isAnnotationPresent('Name') && type.getAnnotation('Name').getArgValue('global') === 'true')
			);
            ...
            if (!ClassUtil.isTopLevel(type.getClass())
				...
				for each (method in methods) {
					var observer:Annotation = method.getAnnotation('Observer');
		...
		public function addSubcontext(name:String):void {
		    if (!isComponent(name) 
		    	...
		    	|| getDescriptor(name).factory.type !== Tide.TYPE_SUBCONTEXT) {
		...
		private function addComponentFromType(modulePrefix:String, name:String, type:Type, global:Boolean, inConversation:Boolean, remote:String, autoCreate:String, restrict:int):void {
			...
			if (type === Tide.TYPE_BASECONTEXT || type.isSubclassOf(Tide.TYPE_BASECONTEXT) || type.isInterface())
            	return;
            
			if (type.isSubclassOf(Tide.TYPE_ICOMPONENT)) {
            ...
            if (type === Tide.TYPE_IENTITY || type.isSubclassOf(Tide.TYPE_IENTITY) || type.alias.length > 0 || type.hasDeclaredConstructor()) {
            ...
            if (type === Tide.TYPE_ILIST || type.isSubclassOf(Tide.TYPE_ILIST)) {
                internalAddComponent(name, type, null, inConversation, autoCreate == "true", restrict, false);
                if (global)
	            	setComponentGlobal(name, true);
                if (remote)
            ...
            if (type === Tide.TYPE_IUICOMPONENT || type.isSubclassOf(Tide.TYPE_IUICOMPONENT)) {
                internalAddComponent(name, type, null, inConversation, autoCreate == "true", restrict, false);
                if (global)
	            	setComponentGlobal(name, true);
                if (remote)
		...
		public function removeComponent(name:String):void {
		    if (isComponent(name) && getDescriptor(name).factory != null && getDescriptor(name).factory.type === Tide.TYPE_SUBCONTEXT)
		        return;
